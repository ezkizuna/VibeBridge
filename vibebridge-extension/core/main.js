...
// waits for the model's reply, parses VibeBridge commands (ZSParse), asks the
// background worker to execute them on the VSCode MCP bridge, and feeds the
...
(() => {
  ...
  const GITHUB_URL = "https://github.com/ezkizuna/VibeBridge/releases";
  ...
  // AI chat sites VibeBridge works on. Keep in sync with manifest.json
  ...
  const A = {
    ...
    // For the KNOWN VSCode vision tool (screen_capture) toolCategory already
  ...
  async function waitForResponse(base) {
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      // VibeBridge command - DeepSeek's DSML invoke/parameter tags (reported by
  ...
  // the model "the local VibeBridge bridge is unreachable", which sent the user
  ...
  // The known VSCode vision tool (screen_capture) is themed "screen" by name via
  ...
  //
  // DEGRADED MODE (VSCode closed, running on an addon server like Blender)
  // is where this used to hurt: a list_tools whose VSCode half is dead blocks the
  ...
  async function runTool(call) {
    ...
    // Virtual command: list the MCP server(s) VibeBridge is currently connected
    ...
    if (name === "list_mcp_servers") {
      ...
      const lines = servers.length
        ? servers.map((sv) => {
            const label = sv.id === "vscode" ? "VSCode (primary)" : `${sv.id} (addon)`;
            return `- ${sv.id}: ${label} - ${sv.alive ? `${sv.tools || 0} commands available` : "offline (no tools)"}`;
          })
        : ["- vscode: VSCode (primary) - unknown (bridge did not report server health)"];
      return (
        `Output of 'list_mcp_servers':\n` +
        `Connected MCP servers (${lines.length}):\n${lines.join("\n")}\n` +
        `Use list_commands with a "server" param (one of the ids above) to see that server's exact commands. Without "server", list_commands defaults to "vscode".`
      );
    }
    // Virtual command: list available commands with full details. Defaults to
    // the primary VSCode server - a DIFFERENT server's tools only ever show up
    ...
    if (name === "list_commands" || name === "list_tools") {
      ...
      if (requested === "vscode") {
        ...
        if (!rbxUsable) {
          const others = srv.filter((x) => x.id !== "vscode" && x.alive && (x.tools || 0) > 0);
          const otherStr = others.length
            ? `Other connected MCP server(s): ${others.map((x) => x.id).join(", ")}. Call list_mcp_servers, then list_commands with a "server" param to use them for anything that does not need VSCode.`
            : `No other MCP server is connected right now.`;
          return `Output of '${name}':\nVSCode is currently OFFLINE (closed, no place open, or its MCP server disabled), so its commands cannot run. This is an environment problem on the user's machine, not your mistake. Tell the user in one short sentence to open their place in VSCode and enable its MCP server. ${otherStr}`;
        }
      }
      ...
      // hiding everything.
      const scoped = A.toolList.filter((t) => (t.server || "vscode") === requested);
      if (!A.toolList.length) return `Output of '${name}':\nNo commands available - the bridge or VSCode may be offline.`;
      if (!scoped.length) {
        return `Output of '${name}':\nERROR: no server named "${requested}" is connected. Connected servers: ${[...known].join(", ") || "vscode"}. Call list_mcp_servers to check.`;
      }
    ...
    // The VSCode MCP REQUIRES datamodel_type on execute_luau (enum Edit/Client/
    ...
    if (r.ok && /Unable to find an active Studio instance|previously active Studio has disconnected/i.test(r.text || "")) {
      ui.banner("warn", "VSCode is not connected",
        "Open your place in VSCode and enable the MCP server (Assistant AI → … → Manage MCP Servers → Enable Studio as MCP Server), then try again.");
      return ZS.FEEDBACK.studioOffline;
    }
    // The VSCode MCP reports missing/invalid required parameters as a SUCCESS
    ...
    // The VSCode MCP also reports Luau PARSE/RUNTIME errors as a SUCCESS whose
    ...
    if (r.ok) {
      ...
      if (r.images && r.images.length) {
        // Show the capture in a left-hand VibeBridge popup (from the in-memory
    ...
    if (r.kind === "stale-extension") {
      ui.banner("warn", "Reload this page",
        "VibeBridge was updated or reloaded while this tab was open, so this page is running an " +
        "old copy of it and commands can no longer run. Reload the page (F5) to reconnect - your " +
        "bridge and VSCode are unaffected.");
    ...
    if (name === "execute_luau") {
      ...
      const hint = err.includes("Failed to parse command code")
        ...
        : err.includes("attempt to") || err.includes("nil value")
          ...
          : "Check your Lua syntax, make sure you use 'return' to output values (not 'print()'), and that all APIs you call exist in the current VSCode context.";
  ...
  // a chip that says nothing: list_vscode_studios read
  ...
  async function agentLoop(base) {
    ...
    try {
      while (!A.stop) {
        ...
        //    working in VSCode) -> still "visible" -> the agent keeps
        ...
        if (res.kind === "tool") {
          ...
          // A tool (VSCode OR any custom MCP server) that actually RETURNED an
          ...
          if (!isErr && call.tool !== "list_commands" && A.toolList.length) {
            ...
            if (A.toolCallsSinceReminder >= REMIND_TOOLS_EVERY) {
              ...
              // Scope the reminder to the primary VSCode server, exactly like
              ...
              // needs the primary VSCode set; addon commands were listed on demand
              // and the bridge routes by name regardless.
              const vscode = A.toolList.filter((t) => (t.server || "vscode") === "vscode");
              toSend += ZS.toolsReminder(vscode) + "\n" + ZS.memoryNudge();
  ...
  function withSysResend(text) {
    ...
    const rider =
      "\n\n────────────────────────────────\n" +
      ZS.RESEND_MARKER + "\n" +
      "(System note from VibeBridge - an automatic re-statement of your operating " +
      ...
      "commands. To be explicit: the VibeBridge extension IS running in this page right " +
  ...
  async function startSession() {
    ...
    try {
      ...
      if (!alive()) return;
      // Hard Start gate (VibeBridge has no degraded start): the primary
      // VSCode server must be alive, otherwise the agent could not touch the
      // project at all. Old bridges don't report per-server health - then
      ...
      if (!A.toolList.length || !_vsUp) {
        ui.banner("warn", "VSCode not ready",
          ...
          "2) Press Cmd+Shift+P (Ctrl+Shift+P on Windows) and run 'VS Code MCP Bridge: Start Server' " +
          "(the status bar shows MCP :3333 when it is on). " +
          "3) Back here, click Start VSCode agent again.");
        return;
      }
      ...
      if (startRes.kind === "tool" && startRes.calls && startRes.calls.length === 1 &&
        ...
        // VSCode down short-circuits list_commands into a plain "offline" note
        ...
        // (the bridge keeps advertising VSCode's catalogue even with no Studio
        // attached, so A.toolList still has 25+ entries that were never actually
        // usable this boot).
        if (/VSCode is currently OFFLINE/.test(toolFeedback)) {
          decorate.toolBox(startRes.item, "Loading commands", "err", "VSCode offline", true);
        } else {
          // Count what the model ACTUALLY received: list_commands is scoped to the
          // primary VSCode server (main.js ~629), so showing A.toolList.length (every
          // connected server merged - VSCode + Blender + addons) overstated the boot
          // count and made it look like all servers were loaded at once. Count the
          // VSCode-scoped tools instead, matching the real result.
          const vscodeCount = A.toolList.filter((t) => (t.server || "vscode") === "vscode").length;
          decorate.toolBox(startRes.item, "Loading commands", "done", `${vscodeCount} commands`, true);
        }
  ...
  const ICONS = {
    ...
    vscode:  SVG('<path d="M12 2 3 7v10l9 5 9-5V7z"/><path d="M3 7l9 5 9-5M12 12v10"/>'),
  ...
  const decorate = {
    ...
    classify(item, next) {
      ...
      // ONLY in a real VibeBridge session (started or bootstrapping). Without
      ...
      if (P.isAssistantItem(item) && ZSParse.hasCommandShape(txt) &&
        ...
        // name alone wouldn't reveal it (parity with VSCode screen_capture). The
  ...
  const ui = (() => {
    ...
    function build() {
      ...
      root.innerHTML = `
        <div id="zs-bar">
          ...
          <span id="zs-brand">VibeBridge <span class="zs-free">v${EXT_VERSION}</span></span>
      ...
      const toggleMenu = () => {
        ...
        if (!menuEl.hidden) {
          ...
          // scrollTop here is a no-op. Deferring one frame lets placeBar's
          // already-queued tick clip the box first, so there's real scroll room
          // by the time we set scrollTop.
    ...
    // ADDONS: the VSCode server stays primary and is never in this list. Each
    ...
    function mergedMcpServers() {
      const live = ((A.bridge && A.bridge.servers) || []).filter((sv) => sv.id !== "vscode");
    ...
    // Derive a config-safe server id from a display name (vscode is reserved).
    function mcpSlug(name) {
      let s = String(name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      if (!s || s === "vscode") s = `addon-${s || "server"}`;
      let id = s, n = 2;
    ...
    // prompt, and the MCP server list. Opens above the bar.
    function buildMenu() {
      ...
      // VSCode always heads the list - greyed out, no health dot (its own status
      // is already the main VibeBridge dot elsewhere) and no remove button (it's
      // the primary server, protected bridge-side too).
      let mcpList =
        `<div class="zs-mcp-item zs-mcp-item-primary"><div class="zs-mcp-info"><span class="zs-mcp-name">VSCode</span><span class="zs-mcp-url">primary - always connected</span></div></div>`;
      ...
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">VibeBridge</span><span class="zs-menu-tag">v${EXT_VERSION}</span></div>
         <section class="zs-menu-sec">
          ...
          <section class="zs-menu-sec">
            ...
            <textarea id="zs-set-text" rows="4" placeholder="e.g. Always explain file edits briefly before writing them."></textarea>
         ...
         <section class="zs-menu-sec">
           ...
           <div class="zs-menu-note">VSCode is always connected (primary). Add another MCP server (e.g. Blender, Sketchfab) as an addon - the bridge restarts briefly to load it. Experimental.</div>
           ${mcpList}
    ...
    function buildSetup() {
      ...
      setupCard.innerHTML =
        `<div id="zs-setup-head"><span id="zs-setup-logo">VibeBridge</span><span id="zs-setup-tag">Setup</span></div>` +
        `<div id="zs-setup-sub">The <b>Bridge</b> is what connects this chat to VSCode. Three steps and you're running.</div>` +
        `<ol id="zs-setup-steps">` +
          `<li>Open your project folder in VSCode and trust it</li>` +
          `<li>Start its MCP server (Cmd+Shift+P), then run the Bridge launcher (<code>start.bat</code> / <code>MacOS_Start.command</code>)</li>` +
          `<li>Back here, click <b>Start VSCode agent</b></li>` +
        `</ol>` +
    ...
    function refreshSetup(bridgeConnected) {
      ...
      // conversation with no VibeBridge session (the "No agent here" state),
    ...
    //  • starting        → spinner, "Starting the VSCode agent…"
    ...
    function renderBar() {
      ...
      // chats share a key, and the conversation id only appears mid-bootstrap).
        ...
        // server's per-server health count (VSCode + addons like Blender) - NOT the
        // VSCode-only count, which made the total drop to just 27 after a reload.
        ...
        if (A.bridge && A.bridge.connected === false) {
          ...
          // VSCode dropped mid-session but another MCP server is still alive.
          // Keep the YELLOW dot as the honest health signal, but do NOT keep the
          // red imperative "open VSCode" nag on screen for the whole
          // session (warn=false → no zs-state-warn red text). The full nag
          // still shows when NO server is usable (the branches below).
          toneClass = "warn";
          msg = `<b>Agent active</b>${tools ? ` · ${tools} tools` : ""} · VSCode offline`;
        } else if (placeDown) {
          toneClass = "warn"; warn = true;
          msg = `<b>Agent active</b> · open a folder in VSCode`;
        } else if (appDown || studioDown) {
          toneClass = "warn"; warn = true;
          msg = studioProcUp
            ? `<b>Agent active</b> · VSCode is open but its MCP server is off - press <b>Cmd+Shift+P</b> and run it`
            : `<b>Agent active</b> · open VSCode & start its MCP server`;
        } else {
          toneClass = "active";
        ...
        if (bridgeOk) {
          ...
          label = "▶︎ Start VSCode agent"; kind = "start";
        } else {
          // VibeBridge has no degraded start: without the primary VSCode
          // server the agent cannot touch the project, so Start stays
          // locked until VSCode is back. The message below always names the
          // exact corrective step.
          toneClass = "warn"; warn = true;
          msg = !A.bridge.connected
            ? `Run <b>start.bat</b> on your PC.`
            : placeDown
              ? `Open a <b>folder</b> in VSCode, then start its MCP server.`
              : (appDown || studioDown) && studioProcUp
                ? `VSCode is open but its MCP server is off - press <b>Cmd+Shift+P</b> and run <b>VS Code MCP Bridge: Start Server</b>.`
                : appDown
                  ? `Open <b>VSCode</b>, open your project folder, then start its MCP server (Cmd+Shift+P).`
                  : studioDown
                    ? `Open <b>VSCode</b>, open your project folder, then start its MCP server (Cmd+Shift+P).`
                    : `Open <b>VSCode</b> for the tools.`;
          label = "▶︎ Start VSCode agent"; kind = "start";
        }
        disabled = !bridgeOk;
      } else {
    ...
    function setStatus(s) {
      ...
      // VibeBridge status tracks ONLY the primary VSCode MCP server. Every other
      // server is an addon and must NEVER make the dot/gate look connected while
      // VSCode itself is down. Old bridges don't send per-server health, so fall
      // back to the aggregate signals they do send (mcpAlive / total tools).
      const vscode = servers.find((x) => x.id === "vscode");
      const mcpUp = vscode ? !!vscode.alive : (!!s.mcpAlive || servers.some((x) => x.alive));
      // VSCode-only count drives the connectivity gate (the dot must never look
      // green off an addon while VSCode itself is down)...
      const vscodeTools = vscode ? (vscode.tools || 0) : (s.tools || 0);
      const mcpOk = s.connected && (mcpUp || vscodeTools > 0);
      // ...but the DISPLAYED count is the aggregate across every server (VSCode +
      // addons like Blender), so it stays consistent with the bar and doesn't
      // under-report when addon servers are loaded.
      const totalTools = servers.reduce((n, x) => n + (x.tools || 0), 0) || s.tools || vscodeTools;
      ...
      // so for the second case "open VSCode" is dead-end advice - the
      ...
      if (!s.connected) txt = "Bridge offline, run start.bat";
        ...
        : "VSCode not connected - open it and enable its MCP server";
      else if (studioOff) txt = "Studio not connected, enable the MCP server in VSCode";
      ...
      appDown = noApp;
      studioProcUp = procUp;
      // A non-VSCode MCP (Blender, Sketchfab, ...) that is actually alive. When
      // VSCode itself is down but such a server is present, the session can still
      // start in a DEGRADED mode - the agent just can't touch VSCode until Studio
      // is back. Gated on s.connected so a dropped bridge never reads as usable.
      addonOk = !!s.connected && servers.some((x) => x.id !== "vscode" && x.alive && (x.tools || 0) > 0);
    ...
    function staleExtensionAlert() {
      ...
      b.innerHTML = `<div class="zs-banner-t">⚠ Reload this page to reconnect VibeBridge</div>
        <div class="zs-banner-m">VibeBridge was updated or reloaded while this tab was open, so this page is still running the old copy and commands can no longer run. Your bridge and VSCode are fine - only this page needs refreshing.</div>
    ...
    // the Start button so they discover how to enable VSCode control.
    let nudged = false;
    function nudgeStart() {
      if (A.started || !P.isFreshChat()) return;
      if (!nudged) {
        nudged = true;
        toast("Tip: click “▶︎ Start VSCode agent” to let the AI control VSCode.");
      }
      if (!actionBtn) return;
    ...
    // Left-hand VibeBridge popup showing the latest screen_capture. Fed from the
    ...
    build();
    return { setStatus, staleExtensionAlert, setStarted, setStarting, showStop, markStopping, inputCover, toast, banner, showImages, nudgeStart, updateStartGate, refreshSetup, getCustomPrompt, getCustomMcpServers, openMenu: () => openMenuFn && openMenuFn() };
  })();
  ...
  // VibeBridge's own UI). A genuine "regenerate ↻" is always such a click;
  ...
  setInterval(() => {
    ...
    // The bare isHardGenerating() term is gated on a live VibeBridge session: on
  ...
  // A conversation IS a VibeBridge session if any rendered turn carries a
  // telltale artefact: the system-prompt marker, an injected tool-result /
  // system-note turn, or a VibeBridge command an assistant wrote. Works even
  ...
  function syncSessionState() {
    ...
    // via the SITE's own new-chat (not VibeBridge's button), the loop is bound to
    ...
    // the fresh chat instead of "Start VSCode agent". The "/app" → "/app/<id>" id
  ...
  log(`VibeBridge content script ready (provider: ${P.id})`);
})();
