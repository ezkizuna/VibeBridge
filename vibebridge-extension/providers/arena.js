...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. VibeBridge's
...
const ZSProvider = (() => {
  ...
  // textareas are recaptcha/aria-hidden). Scope away VibeBridge's own settings
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…VibeBridge: result truncated to fit Arena's input limit - ` +
  ...
  // VibeBridge only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // bounces the tab), which VibeBridge can't drive. We nudge the mode dropdown
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `VibeBridge only works in plain chat. ${um.label} mode uses a different ` +
    ...
    return `Switch the mode dropdown to <b>Direct</b> - VibeBridge only works in ` +
  ...
  // VibeBridge bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored VibeBridge bar (a real, full-width element hugging the
