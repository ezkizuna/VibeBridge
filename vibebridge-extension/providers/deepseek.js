...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match VibeBridge's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole VibeBridge bar INTO the
  ...
  function enforceComposer(reason) {
    ...
    // off) without VibeBridge reverting their choice every frame.
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…VibeBridge: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "vibebridge_..." filename)
          ...
          (/^blob:/.test(im.getAttribute("src") || "") || /^vibebridge_/.test(im.getAttribute("alt") || "")));
    } catch { return []; }
  };
  ...
  //   isStarted():bool        - a VibeBridge session exists in this chat
