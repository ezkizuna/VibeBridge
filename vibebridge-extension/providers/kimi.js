...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: skip VibeBridge's own injected UI (the
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed VibeBridge bar over the editor. The real
  ...
  // its built-in tools over the VibeBridge command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting VibeBridge command blocks.
    return !!(m && /swarm|agent/i.test(m.textContent || ""));
  }
  // Visible mode guard for the VibeBridge bar (core renderBar reads this every
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the VibeBridge commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  // Pick a model that actually speaks the VibeBridge command protocol, ONCE,
  ...
  function fileFromImage(img, i) {
    ...
    return new File([arr], `vibebridge_${Date.now()}_${i}.${ext}`, { type: mime });
  }
  ...
  return {
    ...
    // the capture is shown in the core's left-hand VibeBridge popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the VibeBridge panel.
    // Kimi sometimes reaches for its OWN built-in/native tools (web search, code
    // runner, etc.) instead of emitting the VibeBridge command blocks that drive
    // VSCode - model behavior, not something the prompt fully prevents.
    unstableWarning:
      "Kimi sometimes uses its own native tools instead of the VSCode commands (model behavior, not the extension). " +
      "If it stops acting in VSCode and answers in plain text or runs its own tools, remind it to use the VibeBridge commands - or start a new session.",
