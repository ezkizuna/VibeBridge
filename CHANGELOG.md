...
providers, agent loop, parser, chips, menu and bridge protocol are unchanged -
only the application was swapped: Roblox Studio -> VSCode.

### Changed
- Primary MCP server is now `vscode` (tools/vscode_mcp_server.py via
  launch_vscode_mcp.py) instead of Roblox StudioMCP.
- Status probe reads the VSCode link (jhamama MCP Bridge on :3333 + open
  workspace folder) instead of Studio attach/place state.
- System prompt, tool notes, feedback strings and banners rewritten for
  VSCode workflows (read/write/preview diff/diagnostics/terminal).
- Project memory lives in MEMORY.md at the workspace root.
- StudioMCP-specific recovery (port reclaim, zombie/orphan kills, ropilot
  handling, update detection) removed - those failure modes don't exist here.
